...
EPOCHS = 8
LR = 1e-4
NUM_WORKERS = 0  # Windows için güvenli başlangıç
VAL_RATIO = 0.15
...
def load_class_names():
    ...
    class_names = [str(item[0][0]).strip() for item in food_list]
    return class_names
...
def build_samples():
    ...
    print(f"Test sample sayisi: {len(test_samples)}")
    print(f"Skipped images: {skipped_images}")
    print(f"Skipped instances: {skipped_instances}")

    return class_names, class_to_idx, train_samples, test_samples
...
def main():
    ...
    best_val_acc = 0.0
    best_path = BASE_DIR / "best_resnet50_unimib.pth"
